...
import { z } from "zod";
import { loadSummaries} from "./admin-api-common";
...
export function generatePeopleByPlacesPrompt(): string {
    ...
    for (const [place, people] of peopleByPlace.entries()) {
        prompt += `- ${formatter.format(people.map((person) => person.name))} ${
...
export async function generatePeopleTalkHistory(): Promise<string> {
    const players = WA.players.list();

    const playersArray = Array.from(players);
    const summaries = await loadSummaries(
        1,
        playersArray.map((player) => player.uuid),
        false
    );
    ...
    for (const player of playersArray) {
        const summaryResponse = summaries.find((summary) => summary.interlocutor_uuid === player.uuid);
        if (summaryResponse === undefined) {
            prompt += "You never talked to " + player.name + "\n";
        } else {
            prompt +=
                "You talked to " +
                player.name +
                " on " +
                summaryResponse.created_at.toLocaleDateString("en-us", {
                    weekday: "long",
                    year: "numeric",
                    month: "short",
                    day: "numeric",
                ...
                " at " +
                summaryResponse.created_at.toLocaleTimeString("en-us", { hour: "2-digit", minute: "2-digit" }) +
                "\n";
        }
    }
...
export function generatePeopleByRolePrompt(): string {
    ...
    if (coworkers.length === 0) {
        ...
        prompt = `${formatter.format(
            peopleByRole["coworkers"].map((person) => person.name)
        )} are coworkers. They are part of your team and work in this map.\n`;
    }
